    ...
    def _background_execute(self, game_session: GameSession, user_input: str):
        ...
        try:
            ...
            with DBManager(self.db_path) as thread_db_manager:
                ...
                self.ui_queue.put({"type": "thought_bubble", "content": plan.plan})
                ...
                # Check for end_setup_and_start_gameplay tool call and update game mode

                for result in tool_results:

                    if result.get("tool_name") == EndSetupAndStartGameplay.model_fields["name"].default and result.get(
                ...
                if current_game_mode == "GAMEPLAY" or not any(
                    ...
                    r.get("tool_name") == EndSetupAndStartGameplay.model_fields["name"].default for r in tool_results
