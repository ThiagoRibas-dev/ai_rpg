...
class WorldInfoService:
    ...
    def ensure_indexed(self, prompt_id: int | None):
        ...
        try:
            ...
            self.logger.warning(f"World Info indexing failed for prompt {prompt_id}: {e}")

    def search_for_history(self, prompt_id: int | None, recent_messages: List[Message], k: int = 4) -> list[str]:
        if not prompt_id:
            return []
        try:
            ...
            self.logger.warning(f"World Info retrieval failed for prompt {prompt_id}: {e}")
            return []
