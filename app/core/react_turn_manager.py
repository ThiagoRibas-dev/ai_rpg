...
class ReActTurnManager:
    ...
    def execute_turn(self, game_session: GameSession, thread_db_manager, turn_id: str):
        ...
        try:
            # Use working_history which includes tool calls & results
            final_history = list(working_history)
            
            # The narrative text might not be in working_history if it was generated by the fallback
            ...
            final_out = self.llm_connector.get_structured_response(
                ...
                output_schema=TurnFinalOutput,
                temperature=0.7,
            )
            choices = list(final_out.choices or [])
            turn_summary = (final_out.summary or "").strip()
            
            if final_out.tags:
                turn_tags = [
                    t.strip() for t in final_out.tags if isinstance(t, str) and t.strip()
                ]
            turn_importance = int(final_out.importance)
            
        except Exception as e:
            self.logger.warning(f"Turn final output (choices/metadata) generation failed: {e}")
