...
class ToolExecutor:
    ...
    """
    def __init__(self, tool_registry, db_manager, vector_store, ui_queue: queue.Queue | None = None, logger: logging.Logger | None = None):
    ...
    def execute(self, tool_calls: List[BaseModel], session, tool_budget: int, current_game_time: str | None = None) -> Tuple[List[Dict[str, Any]], bool]:
        """
        ...
        for i, call in enumerate(tool_calls[:tool_budget]):
            ...
            call_type = type(call)
            self.logger.debug(f"Tool call {i}: type={call_type.__name__}, is BaseModel={call_type is BaseModel}")
            
            # âœ… Defensive check: ensure it's not a raw BaseModel
            if call_type is BaseModel or not hasattr(call, 'name'):
                self.logger.error(f"Invalid tool call at index {i}: {call_type} - {call}")
                results.append({
                    "tool_name": "unknown",
                    "arguments": {},
            ...
            try:
                ...
                self.logger.error(f"Tool call has no 'name' attribute: {call_type} - {e}")
                results.append({
                    "tool_name": "unknown",
                    "arguments": {},
            ...
            try:
                ...
                if isinstance(call, (schemas.MemoryUpsert, schemas.MemoryQuery, 
                                    schemas.MemoryUpdate, schemas.MemoryDelete)):
