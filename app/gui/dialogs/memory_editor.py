...
class MemoryEditorDialog:
    ...
    def open(self):
        with (
            self.dialog,
            ui.card().classes("w-[600px] bg-slate-900 border border-slate-700"),
        ):
            ui.label(f"Edit Memory #{self.memory.id}").classes(
            ...
            with ui.grid(columns=2).classes("w-full gap-4"):
                ...
                ui.number(
                    label="Priority (1-5)", min=1, max=5, format="%.0f"
                ).bind_value(self, "priority").classes("w-full")

            ui.input(label="Tags (comma separated)").bind_value(
                self, "tags_str"
            ).classes("w-full")

            ui.textarea(label="Content").bind_value(self, "content").classes(
                "w-full"
            ).props("rows=6")
