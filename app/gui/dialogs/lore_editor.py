...
class LoreEditorDialog:
    ...
    def open(self):
        with (
            self.dialog,
            ui.card().classes(
                "w-[900px] h-[700px] bg-slate-900 border border-slate-700 p-0"
            ),
        ):
            # Header
            with ui.row().classes(
                "w-full bg-slate-950 p-4 justify-between items-center border-b border-slate-800"
            ):
            ...
            with ui.row().classes("w-full h-full gap-0"):
                ...
                with ui.column().classes("w-1/3 h-full border-r border-slate-800 p-2"):
                    ui.button(
                        "New Entry", icon="add", on_click=self.create_new
                    ).classes("w-full bg-slate-800 mb-2")
                ...
                with ui.column().classes("w-2/3 h-full p-4 gap-4"):
                    ...
                    with self.edit_container:
                        ...
                        ui.input(label="Tags").bind_value(self, "edit_tags").classes(
                            "w-full"
                        )
                        ui.textarea(label="Content").bind_value(
                            self, "edit_content"
                        ).classes("w-full flex-grow text-sm").props("outlined")

                        with ui.row().classes("w-full justify-end"):
                            ui.button(
                                "Delete", on_click=self.delete_current, color="red"
                            ).props("flat")
                            ui.button(
                                "Save Changes", on_click=self.save_current
    ...
    def refresh_list(self):
        ...
        with self.list_container:
            for mem in memories:
                ...
                title = (
                    (mem.content[:30] + "...") if len(mem.content) > 30 else mem.content
                )
                bg = (
                    "bg-slate-700"
                    if self.selected_mem and self.selected_mem.id == mem.id
                    else "bg-transparent"
                )

                ui.button(title, on_click=lambda m=mem: self.select_entry(m)).classes(
                    f"w-full text-left text-xs truncate {bg} hover:bg-slate-800 p-2 rounded"
                )
