...
class QuestInspector:
    ...
    def refresh(self):
        ...
        with self.container:
            if not quests:
                ui.label("No Active Quests").classes(
                    "text-gray-500 italic text-center w-full mt-4"
                )
                return
    ...
    def _render_quest_card(self, quest):
        ...
        with ui.card().classes(
            f"w-full bg-slate-800 border-l-4 {border_color} p-3 mb-2"
        ):
            with ui.row().classes("w-full justify-between items-start"):
                with ui.column().classes("gap-1"):
                    with ui.row().classes("items-center gap-2"):
                        ui.icon(icon).classes(
                            f"text-lg {'text-green-400' if is_completed else 'text-amber-400'}"
                        )
                        ui.label(quest.get("title", "Unknown Quest")).classes(
                            "font-bold text-md"
                        )

                    ui.label(quest.get("description", "")).classes(
