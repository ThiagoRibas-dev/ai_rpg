...
class CharacterInspectorView(ctk.CTkFrame):
    ...
    def __init__(self, parent, db_manager):  # Changed to db_manager
        ...
        self.character_selector = ctk.CTkOptionMenu(
            selector_frame, values=["player"], command=self._on_character_selected
        )
        self.character_selector.pack(side="left", padx=5, fill="x", expand=True)

        # Scrollable content
        self.scroll_frame = ctk.CTkScrollableFrame(
            self, fg_color=Theme.colors.bg_secondary
        )
    ...
    def _render_character(self):
        ...
        if attributes:
            ...
            ctk.CTkLabel(
                attr_frame, text="⚔️ Attributes", font=Theme.fonts.body, anchor="w"
            ).pack(fill="x", padx=5, pady=5)

            for key, value in attributes.items():
                self._add_key_value(
                    attr_frame, key.replace("_", " ").title(), str(value)
                )
        ...
        if conditions:
            ...
            ctk.CTkLabel(
                cond_frame, text="🩹 Conditions", font=Theme.fonts.body, anchor="w"
            ).pack(fill="x", padx=5, pady=5)
        ...
        if location:
            loc_label = ctk.CTkLabel(
                self.scroll_frame, text=f"📍 Location: {location}", anchor="w"
            )
        ...
        if inventory:
            inv_label = ctk.CTkLabel(
                self.scroll_frame, text=f"🎒 Inventory: {inventory}", anchor="w"
            )
...
class InventoryInspectorView(ctk.CTkFrame):
    ...
    def __init__(self, parent, db_manager):  # Changed to db_manager
        ...
        self.owner_selector = ctk.CTkOptionMenu(
            selector_frame, values=["inventory:player"], command=self._on_owner_selected
        )
        self.owner_selector.pack(side="left", padx=5, fill="x", expand=True)

        # Header with capacity
        self.header_label = ctk.CTkLabel(
            self, text="🎒 Inventory (0/10)", font=Theme.fonts.subheading
        )
        ...
        ctk.CTkButton(
            button_frame, text="🔄 Refresh", command=self.refresh, height=30
    ...
    def _create_item_card(self, item):
        ...
        ctk.CTkLabel(
            header_frame, text=name_text, font=Theme.fonts.body, anchor="w"
...
class QuestInspectorView(ctk.CTkFrame):
    ...
    def __init__(self, parent, db_manager):  # Changed to db_manager
        ...
        # Refresh button
        ctk.CTkButton(self, text="🔄 Refresh", command=self.refresh, height=30).pack(
