...
class ChatComponent:
    ...
    def render(self):
        ...
        with ui.column().classes('w-full p-0 gap-0 border-t border-slate-700'):
            ...
            # Navigation Bar
            self.nav_container = ui.row().classes('w-full justify-center gap-2 p-2 bg-slate-900 border-b border-slate-800')
            ...
            with ui.row().classes('w-full p-4 ' + Theme.bg_tertiary + ' items-end'):
                ...
                # Global History Menu
                with ui.button(icon='history').props('flat round dense').classes('text-gray-400'):
                    with ui.menu():
                        ui.menu_item('Reload History', on_click=self.load_history)
                        ui.separator()
                        ui.menu_item('Clear Chat View', on_click=lambda: self.container.clear())
                ...
                # Send Button
                self.send_btn = ui.button(icon='send', on_click=self.handle_enter) \
                    .props('flat round dense') \
                    .classes(Theme.text_accent)
                
                # Stop Button (Hidden by default)
                self.stop_btn = ui.button(icon='stop', on_click=self.handle_stop) \
                    .props('flat round dense') \
    ...
    def _render_interactive_message(self, index: int, msg, name: str):
        ...
        if msg.role == 'thought':
            with ui.row().classes('w-full justify-start'):
                with ui.card().classes('bg-yellow-900/20 border border-yellow-700/50 p-2 w-full max-w-3xl'):
        ...
        with chat_msg:
            ...
            content_container = ui.column().classes('w-full p-0 m-0 gap-2 min-w-[200px]')
            with content_container:
                ui.markdown(msg.content).classes('text-base leading-relaxed')
                with ui.row().classes('opacity-0 hover:opacity-100 transition-opacity absolute -right-2 -top-2 bg-slate-800 rounded-full shadow-sm'):
                    ui.button(icon='edit', on_click=lambda: self._toggle_edit_mode(content_container, index, msg)).props('flat dense round size=xs')

    def _toggle_edit_mode(self, container, index, msg):
        container.clear()
        with container:
            editor = ui.textarea(value=msg.content).classes('w-full min-w-[300px] bg-slate-900 p-2 rounded').props('autogrow input-class="text-sm"')
            with ui.row().classes('w-full justify-end gap-2'):
                ui.button('Cancel', on_click=lambda: self.load_history()).props('flat dense size=sm')
                ui.button('Save', on_click=lambda: self._save_edit(index, editor.value)).classes('bg-green-600 text-white').props('dense size=sm')
    ...
    def add_system_message(self, text):
        ...
        with self.container:
            with ui.row().classes('w-full justify-center my-2'):
                ui.label(text).classes('text-gray-500 italic text-xs bg-slate-900 px-3 py-1 rounded-full')
        self._scroll_down()
    ...
    def add_location_banner(self, location_data: dict):
        ...
        with self.container:
            with ui.row().classes('w-full justify-center'):
                with ui.card().classes('w-full max-w-2xl bg-slate-950 border-y-2 border-amber-600 p-4 items-center text-center'):
                    ui.label(f"📍 {name}").classes('text-xl font-bold text-amber-500 uppercase tracking-widest')
                    if desc:
                        ui.label(desc).classes('text-sm text-gray-400 italic mt-2 font-serif')
        self._scroll_down()

    def update_navigation(self, exits: list):
        if not self.nav_container: return
        ...
        with self.nav_container:
            ui.label("EXITS:").classes('text-xs font-bold text-gray-500 self-center mr-2')
            for direction in exits:
                ui.button(direction.title(), on_click=lambda d=direction: self.handle_choice(f"Go {d}")) \
                    .props('outline dense size=sm') \
                    .classes('text-gray-300 border-gray-600 hover:bg-slate-800')
