...
class MapComponent:
    ...
    def render(self):
        with ui.column().classes('w-full p-0 gap-0'):
            with ui.row().classes('w-full bg-slate-900 p-2 items-center justify-between border-b border-slate-700'):
                with ui.row().classes('gap-1'):
                    ui.icon('map').classes('text-gray-400')
                    ui.label('Visuals').classes('font-bold text-gray-200')
                
                with ui.button_group().props('flat'):
                    ui.button('Tactical', on_click=lambda: self.set_mode('tactical')).props('dense')
                    ui.button('World', on_click=lambda: self.set_mode('world')).props('dense')

            # ✅ FIX: Added sanitize=False
            self.content = ui.html('', sanitize=False).classes('w-full bg-slate-950 flex justify-center items-center overflow-hidden')
    ...
    def _generate_tactical_svg(self):
        ...
        svg_parts = [f'<svg width="100%" height="{svg_h}" viewBox="0 0 {svg_w} {svg_h}" xmlns="http://www.w3.org/2000/svg">']
        ...
        for y in range(h):
            for x in range(w):
                ...
                svg_parts.append(f'<rect x="{cx}" y="{cy}" width="{cell_size}" height="{cell_size}" fill="{fill}" stroke="{stroke}" />')
                svg_parts.append(f'<text x="{cx+2}" y="{cy+10}" font-size="8" fill="#64748b" font-family="monospace">{coord}</text>')
                ...
                if coord in entities:
                    ...
                    svg_parts.append(f'<circle cx="{cx+25}" cy="{cy+25}" r="15" fill="{circle_fill}" />')
                    svg_parts.append(f'<text x="{cx+25}" y="{cy+30}" font-size="14" fill="white" text-anchor="middle" font-weight="bold">{initial}</text>')
    ...
    def _generate_world_svg(self):
        ...
        cx, cy = 200, 150
        radius = 100
        svg_parts = [f'<svg width="100%" height="300" viewBox="0 0 400 300" xmlns="http://www.w3.org/2000/svg">']
        ...
        if count > 0:
            ...
            for i, (direction, data) in enumerate(neighbors.items()):
                ...
                svg_parts.append(f'<line x1="{cx}" y1="{cy}" x2="{nx}" y2="{ny}" stroke="#475569" stroke-width="2" />')
                
                name = data.get("display_name", "???")
                svg_parts.append(f'<circle cx="{nx}" cy="{ny}" r="25" fill="#334155" stroke="#94a3b8" stroke-width="2" />')
                svg_parts.append(f'<text x="{nx}" y="{ny+40}" font-size="10" fill="#cbd5e1" text-anchor="middle">{name[:10]}</text>')
                svg_parts.append(f'<text x="{(cx+nx)/2}" y="{(cy+ny)/2}" font-size="9" fill="#64748b" text-anchor="middle" background="black">{direction}</text>')

        svg_parts.append(f'<circle cx="{cx}" cy="{cy}" r="35" fill="#d97706" stroke="#fbbf24" stroke-width="3" />')
        svg_parts.append(f'<text x="{cx}" y="{cy+5}" font-size="12" fill="white" text-anchor="middle" font-weight="bold">{center_key[:8]}</text>')
