...
class ContextEditor:
    ...
    def refresh(self):
        if not self.text_area or not self.session_id: return
    ...
    def render(self):
        with ui.column().classes('w-full gap-2 mt-4 border-t border-slate-700 pt-4'):
            with ui.row().classes('w-full justify-between items-center'):
                ui.label("Author's Note").classes('text-sm font-bold text-gray-400 uppercase')
                self.status_label = ui.label("").classes('text-xs text-green-400')
            
            ui.label("Instructions injected into every turn.").classes('text-xs text-gray-600 italic')
            
            self.text_area = ui.textarea(placeholder="e.g. The NPCs should be suspicious...") \
                .classes('w-full bg-slate-900 text-sm') \
                .props('rows=5 rounded outlined input-class="text-gray-300"')
            
            ui.button("Save Note", on_click=self.save).classes('w-full bg-slate-700 hover:bg-slate-600 text-xs')