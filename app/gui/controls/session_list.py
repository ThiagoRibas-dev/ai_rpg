...
class SessionListComponent:
    ...
    def render(self):
        ...
        # Lorebook Button
        ui.button("Manage Lorebook", icon='book', on_click=self.open_lorebook) \
            .classes('w-full mt-2 bg-purple-900/50 border border-purple-700 hover:bg-purple-900')
    ...
    def refresh(self):
        ...
        with self.container:
            ...
            for sess in sessions:
                ...
                with ui.row().classes(f'w-full p-2 rounded cursor-pointer {bg} items-center justify-between group'):
                    
                    # Clickable Area
                    with ui.row().classes('flex-grow items-center gap-2').on('click', lambda s=sess: self.load_session(s)):
                    ...
                    with ui.button(icon='more_vert').props('flat dense round size=sm').classes('text-gray-400 opacity-0 group-hover:opacity-100'):
                        with ui.menu():
                            ui.menu_item('Load', on_click=lambda s=sess: self.load_session(s))
                            ui.menu_item('Rename', on_click=lambda s=sess: self.rename_session(s))
                            ui.menu_item('Clone', on_click=lambda s=sess: self.clone_session(s))
                            ui.separator()
                            ui.menu_item('Delete', on_click=lambda s=sess: self.confirm_delete(s)).classes('text-red-400')
    ...
    def load_session(self, session):
        ...
        if self.map_component: self.map_component.set_session(session.id)
        ...
        try:
            ...
            for t in turns:
                ...
                new_turn_id = self.db.turn_metadata.create(
                    ...
                    summary=t['summary'],
                    tags=t['tags'], # get_all returns list/dict, create handles json dump
    ...
    def confirm_delete(self, session):
        with ui.dialog() as dialog, ui.card():
            ...
            with ui.row():
                ui.button('Cancel', on_click=dialog.close).props('flat')
                ui.button('Delete', on_click=lambda: self._delete_action(session, dialog)).classes('bg-red-600')
        dialog.open()
    ...
    def _delete_action(self, session, dialog):
        ...
        if self._active_session and self._active_session.id == session.id:
            self._active_session = None
            if self.chat_component: self.chat_component.container.clear()
