...
class FieldEditorDialog:
    ...
    def open(self):
        ...
        try:
            logger.debug(
                f"FieldEditorDialog.open() called for {self.path}, prefab={self.prefab}"
            )
            with ui.dialog() as self.dialog, ui.card().classes(
                "p-4 min-w-[300px] bg-slate-900 border border-slate-700"
            ):
                ui.label(f"Edit: {self.label}").classes(
                    "text-lg font-bold mb-2 text-white"
                )
                ui.label(f"Path: {self.path}").classes(
                    "text-xs text-gray-400 mb-4 font-mono"
                )
                ...
                with ui.row().classes("w-full justify-end gap-2 mt-4"):
                    ...
                    ui.button("Save", on_click=self._handle_save).classes(
        ...
        if self.prefab == PrefabID.VAL_INT:
            ...
            with ui.column().classes("w-full gap-2"):
                ...
                val = (
                    self.current_value
                    if isinstance(self.current_value, dict)
                    else {curr_key: 0, max_key: 0}
                )
                self.input_ref["current"] = (
                    ui.number(
                        label="Current",
            ...
            self.input_ref = (
                ...
                .props('dark standout autogrow input-class="font-mono"')
            )

        else:
            # Fallback: Generic text input
            ui.label(
                f"Unsupported Prefab: {self.prefab}. Using text fallback."
            ).classes("text-yellow-500 text-xs mb-2")
            self.input_ref = (
                ui.input(
                    label="Value",
    ...
    def _extract_value(self) -> Any:
        ...
        if self.prefab in [
            ...
            PrefabID.VAL_BOOL,
            PrefabID.VAL_TEXT,
            PrefabID.VAL_STEP_DIE,
            PrefabID.RES_COUNTER,
        ]:
            ...
            # Fallback
            return getattr(self.input_ref, "value", None)
