...
class PromptDialog(ctk.CTkToplevel):
    ...
    def _create_widgets(self):
        ...
        self.template_textbox = ctk.CTkTextbox(
            main_frame, height=200, font=Theme.fonts.body, wrap="word"
        )
    ...
    def _generate_template(self):
        ...
        if not rules_text:
            self.generate_status.configure(
                text="⚠️ Please enter rules document first", text_color="orange"
            )
            return
        ...
        thread = threading.Thread(
            target=self._generate_template_background, args=(rules_text,), daemon=True
        )
        thread.start()
        ...
        try:
            ...
            generator = TemplateGenerationService(
                ...
                status_callback=lambda msg: self.after(
                    0, self._update_generation_status, msg
                ),
            )
    ...
    def _on_template_generated(self, template: dict | None, error: str | None):
        ...
        if error:
            self.generate_status.configure(text=f"❌ Error: {error}", text_color="red")
            return
        ...
        if template:
            ...
            # Calculate property count for status using the REFINED structure
            prop_count = 0

            # 1. Count Ruleset items
            ruleset = template.get("ruleset", {})
            # Mechanics (The Compendium entries)
            prop_count += len(ruleset.get("mechanics", []))

            # 2. Count Stat Template items
            stat_template = template.get("stat_template", {})
            prop_count += len(stat_template.get("fundamental_stats", []))
            prop_count += len(stat_template.get("derived_stats", []))
            prop_count += len(stat_template.get("vital_resources", []))
            prop_count += len(stat_template.get("consumable_resources", []))
            prop_count += len(stat_template.get("skills", []))
            prop_count += len(stat_template.get("features", []))
            prop_count += len(stat_template.get("identity_categories", []))

            # Equipment Slots
            equip = stat_template.get("equipment", {})
            prop_count += len(equip.get("slots", []))

            self.generate_status.configure(
                text=f"✅ Generated {prop_count} properties", text_color="green"
            )
