...
class SetupWizard(ctk.CTkToplevel):
    ...
    """
    def __init__(self, parent, db_manager, orchestrator, prompt: Prompt, session_manager):
    ...
    def _run_extraction_thread(self):
        ...
        try:
            ...
            if not stat_template:
                # Create a generic one on the fly so extraction doesn't crash
                from app.setup.scaffolding import _get_default_scaffolding
                _, stat_template = _get_default_scaffolding()

            # 2. Run Extractions
            # Ideally parallel, but sequential is safer for SQLite/State for now
            
            # Update UI status (via main thread ideally, but simple label update might work or be ignored)
            # We rely on the progress bar just moving indefinitely or we can use .after calls to update text.
            
            self.after(0, lambda: self.lbl_processing.configure(text="Extracting World Context..."))
            logger.info("Wizard: Extracting World...")
            self.extracted_world = service.extract_world_data(self.input_world_text)
            
            self.after(0, lambda: self.lbl_processing.configure(text="Analyzing Character..."))
            logger.info("Wizard: Extracting Character...")
            self.extracted_char = service.extract_character_data(self.input_char_text, stat_template)
            
            self.after(0, lambda: self.lbl_processing.configure(text="Writing Opening Scene..."))
            ...
            logger.error("Wizard Extraction Failed", exc_info=True)
            self.after(0, lambda: self._on_extraction_error("An unexpected error occurred during extraction."))
    ...
    def _render_current_step(self):
        ...
        if self.step == 0:
            ...
            self.next_btn.configure(text="Generate", state="normal", command=self._start_extraction)
        elif self.step == 1:
            ...
            self.next_btn.configure(text="Start Game", state="normal", command=self._finish)
    ...
    def _render_step_input(self):
        ...
        ctk.CTkLabel(
            header_frame, 
            text=f"New Game: {self.prompt.name}", 
        ...
        ctk.CTkLabel(world_frame, text="ðŸŒ  The World", font=Theme.fonts.subheading).pack(pady=10)
        ...
        ctk.CTkLabel(char_frame, text="ðŸ‘¤ The Protagonist", font=Theme.fonts.subheading).pack(pady=10)
        
        # POLISH: Dynamic System Hint
        hint_text = "Describe your character. Mention their role, history, and strengths."
    ...
    def _randomize_world(self):
        ...
        # Stub: In future, use LLM or genre-based list
        import random
        presets = [
    ...
    def _render_step_processing(self):
        ...
        self.lbl_processing = ctk.CTkLabel(self.main_container, text="Initializing Extraction...", font=Theme.fonts.heading)
    ...
    def _render_step_review(self):
        """Step 3: Review Data."""
        ctk.CTkLabel(self.main_container, text="Step 3: Review & Edit", font=Theme.fonts.heading).pack(pady=10)
        ...
        self.entry_name.bind("<KeyRelease>", lambda _: setattr(self.extracted_char, "name", self.entry_name.get()))
        ...
        self.text_vis.bind("<KeyRelease>", lambda _: setattr(self.extracted_char, "visual_description", self.text_vis.get("1.0", "end-1c")))

        # Stats Editor (Dynamic)
        ctk.CTkLabel(char_scroll, text="Stats (Inferred):", font=Theme.fonts.subheading, anchor="w").pack(fill="x", pady=5)
        ...
        for key, val in self.extracted_char.suggested_stats.items():
            ctk.CTkLabel(self.stats_frame, text=key).grid(row=row, column=0, padx=5, pady=2, sticky="e")
            ...
            entry.bind("<KeyRelease>", lambda _, k=key, ent=entry: self._update_stat(k, ent.get()))
            self.stat_entries[key] = entry
            row += 1
        self.stats_frame.grid_columnconfigure(1, weight=1)

        # Inventory
        ctk.CTkLabel(char_scroll, text="Inventory (One item per line):", anchor="w").pack(fill="x", pady=5)
        ...
        ctk.CTkLabel(world_scroll, text="Starting Location:", font=Theme.fonts.subheading, anchor="w").pack(fill="x")
        ...
        self.entry_loc_name.insert(0, self.extracted_world.starting_location.name_display)
        self.entry_loc_name.bind("<KeyRelease>", lambda _: setattr(self.extracted_world.starting_location, "name_display", self.entry_loc_name.get()))

        ctk.CTkLabel(world_scroll, text="Visual Description (Card):", anchor="w").pack(fill="x")
        self.text_loc_vis = ctk.CTkTextbox(world_scroll, height=80)
        self.text_loc_vis.pack(fill="x", pady=(0,5))
        self.text_loc_vis.insert("1.0", self.extracted_world.starting_location.description_visual)
        self.text_loc_vis.bind("<KeyRelease>", lambda _: setattr(self.extracted_world.starting_location, "description_visual", self.text_loc_vis.get("1.0", "end-1c")))

        ctk.CTkLabel(world_scroll, text="Lore Facts (One per line):", anchor="w").pack(fill="x", pady=5)
        ...
        # Lore update logic is complex (list of objects), we'll parse it on Finish

        # --- TAB 3: OPENING ---
        tab_opening = tabview.add("Opening")
        
        ctk.CTkLabel(tab_opening, text="Opening Scene (Editable):", font=Theme.fonts.subheading).pack(pady=5)
        ...
        self.text_opening.bind("<KeyRelease>", lambda e: setattr(self, "generated_opening", self.text_opening.get("1.0", "end-1c")))
