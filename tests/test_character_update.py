...
def setup_character(mock_db_manager, session_id, initial_character_data):
    mock_db_manager.set_game_state_entity(
        session_id, "character", "player", initial_character_data
    )
...
def test_valid_core_attribute_update(mock_db_manager, session_id, setup_character):
    ...
    updated_char = Character(
        **mock_db_manager.get_game_state_entity(session_id, "character", "player")
    )
    assert updated_char.attributes.hp_current == 50


def test_valid_custom_property_update(
    mock_db_manager, session_id, setup_character, setup_schema_extensions
):
...
def test_type_mismatch_fails_gracefully(
    mock_db_manager, session_id, setup_character, setup_schema_extensions
):
    updates = [{"key": "Sanity", "value": "low"}]  # Sanity is an integer
    with pytest.raises(
        ValueError,
        match="Property 'Sanity' must be of type 'resource', but received 'str'.",
    ):
...
def test_range_violation_fails_gracefully(
    mock_db_manager, session_id, setup_character, setup_schema_extensions
):
    updates = [{"key": "Sanity", "value": 150}]  # Max Sanity is 100
    with pytest.raises(
        ValueError, match="Property 'Sanity' cannot exceed 100. Received 150."
    ):
        character_update_handler(
            character_key="player",
            updates=updates,
...
def test_enum_violation_fails_gracefully(
    mock_db_manager, session_id, setup_character, setup_schema_extensions
):
    updates = [{"key": "Alignment", "value": "Good"}]  # Not in allowed values
    with pytest.raises(
        ValueError,
        match="Property 'Alignment' must be one of \\['Lawful Good', 'Neutral', 'Chaotic Evil'\\]. Received 'Good'.",
    ):
        character_update_handler(
            character_key="player",
            updates=updates,
...
def test_death_logic_triggers_unconscious(
    mock_db_manager, session_id, initial_character_data
):
...
def test_healing_removes_death_conditions(
    mock_db_manager, session_id, initial_character_data
):
...
def test_undefined_custom_property_allowed_with_warning(
    mock_db_manager, session_id, setup_character
):
