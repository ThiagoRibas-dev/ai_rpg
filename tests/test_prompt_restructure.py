...
from app.services.entity_index import smart_truncate, render_index
from app.prefabs.manifest import SystemManifest, EngineConfig, FieldDef
from app.models.vocabulary import PrefabID, MemoryKind
...
class TestPromptRestructure(unittest.TestCase):
    ...
    def test_render_index_table(self):
        ...
        # Headers
        self.assertIn("| Type | Entity ID | Summary |", res)
        # Rows
        self.assertIn("| Location | `town_1` | A sunny village near the sea. |", res)
        self.assertIn("| NPC | `guard_1` | A stern man with a rusty sword. |", res)
        self.assertIn("| Lore | - | The history of the world is long. |", res)
